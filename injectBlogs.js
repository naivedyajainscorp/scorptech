...
blogs.forEach(blog => {
  ...
  blog.sections.forEach((sec, index) => {
    ...
    const imageBlock = sec.image
      ? `<div class="s-blog-sec-img-ph">
           <img src="${sec.image}" alt="${sec.title || 'Section image'}" loading="lazy">
         </div>`
      : `<div class="s-blog-sec-img-ph" style="background:linear-gradient(140deg,#0a1628 0%,#003d7a 50%,#0066cc 100%)">
           Section Image &nbsp;·&nbsp; ${sec.title} &nbsp;·&nbsp; 16 : 9
         </div>`;

    const sectionHTML = `
      <div class="s-blog-section" id="${secId}">
        <div class="s-blog-sec-hdr">
          ${imageBlock}
          <div class="s-blog-sec-hdr-overlay"></div>
          <h2>${sec.title}</h2>
        </div>
